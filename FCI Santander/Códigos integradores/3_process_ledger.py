...
from config import OUTPUT_EXCEL_PATH, TEMPLATE_EXCEL_PATH, JSON_DATA_PATH, VIBRANT_COLORS, log
...
def process_ledger():
    ...
    # Support new wrapper format: {fund_name, movements, ...}
    if isinstance(raw, dict):
        new_movs = raw.get('movements', [])
        initial_balance = raw.get('initial_balance', 0.0)
        fund_label = raw.get('fund_name', 'Santander')
    else:
        new_movs = raw
        initial_balance = 0.0
    ...
    all_movs = []
    if initial_balance > 0.001 and new_movs:
        first_date = new_movs[0].get('date', '01/01/2025')
        all_movs.append({
            'date': first_date,
            'type': 'SUSCRIPCION',
            'cp': initial_balance,
            'amount': initial_balance,  # Placeholder (price = 1.0)
            'source': 'initial_balance',
        })
    all_movs.extend(new_movs)

    to_process = []
    # Use earliest movement date as fallback start
        ...
        if mov['type'] in ('COMPRA', 'SUSCRIPCION'):
            ...
            while needed > 0.0001:
                ...
                if sub['remaining_cp'] >= needed:
                    consume = needed
                    sub['remaining_cp'] = round(sub['remaining_cp'] - needed, 2)
                    needed = 0
                else:
                    consume = sub['remaining_cp']
                    needed = round(needed - consume, 2)
                    sub['remaining_cp'] = 0
                    active_sub_index += 1
    ...
    # Border templates
    border_w = Border(left=med_side)
    border_inner = Border(left=med_side, right=med_side)
    border_ag = Border(right=med_side)
    
    # Apply number formats, fonts, borders, and alignments to all data rows
    ...
    for r in range(5, last_data_row + 1):
        ...
        for c in ['AA', 'AB', 'AC', 'AD', 'AE', 'AF', 'AG']:
            ws[f'{c}{r}'].font = data_font
            ws[f'{c}{r}'].alignment = right_align
            
        # Apply exact template borders
        ws[f'V{r}'].border = border_w
        ws[f'W{r}'].border = border_w
        for c in ['X', 'Y', 'Z', 'AA', 'AB', 'AC', 'AD', 'AE', 'AF']:
            ws[f'{c}{r}'].border = border_inner
        ws[f'AG{r}'].border = border_ag

    # Ensure the very last row has a bottom border to close the Cuadro
    if last_data_row >= 5:
        border_w_bot = Border(left=med_side, bottom=med_side)
        border_inner_bot = Border(left=med_side, right=med_side, bottom=med_side)
        border_ag_bot = Border(right=med_side, bottom=med_side)

        ws[f'V{last_data_row}'].border = border_w_bot
        ws[f'W{last_data_row}'].border = border_w_bot
        for c in ['X', 'Y', 'Z', 'AA', 'AB', 'AC', 'AD', 'AE', 'AF']:
            ws[f'{c}{last_data_row}'].border = border_inner_bot
        ws[f'AG{last_data_row}'].border = border_ag_bot
